...
public final class StringReassembly {
    ...
    public static void addToSetAvoidingSubstrings(Set<String> strSet,
        ...
        //If the set has no substring pairs, then check if there is a
        //SUPERSTRING for str
        ...
        //If the set has no substring paris and no superstring, adds str to set
        //and removes any substring of str.
        if (noSuper) {
    ...
    public static Set<String> linesFromInput(SimpleReader input) {
        ...
        Set<String> value = new Set1L<>();

        //Scans the input stream
        while (!input.atEOS()) {
            String line = input.nextLine();
            //Checks if the whole line is a substring of another
            addToSetAvoidingSubstrings(value, line);
        }
    ...
    public static void printWithLineSeparators(String text, SimpleWriter out) {
        ...
        //Replaces each ~ with a new line escape sequence
        String changed = text.replaceAll("~", "\n");
        out.println(changed);
